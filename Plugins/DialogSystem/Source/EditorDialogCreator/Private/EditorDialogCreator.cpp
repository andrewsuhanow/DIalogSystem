...
TSharedRef<SDockTab> FEditorDialogCreatorModule::OnSpawnPluginTab(const FSpawnTabArgs& SpawnTabArgs)
{
	...
	//TSharedPtr<SDockTab> DialogCreatorWindow111;
	//TSharedPtr<SDialogWindowWidget> DialogWindowWidget;
	//SAssignNew(DialogCreatorWindow111, SDockTab)
	//return SAssignNew(DialogCreatorWindow111, SDockTab)
...
/*			SNew(SBox)
		...
		*/		//-----------------------------
			...
			//SAssignNew(DialogWindowWidget, SDialogWindowWidget)
				.PathToFile(&PathToCurrentXML)
			//.Text(WidgetText)
		];
		
		//FVector2D ooo = DialogCreatorWindow111.Get()->GetDesiredSize();
		//UE_LOG(LogTemp, Display, TEXT("Viwe  x= %f,      x= %f"), ooo.X, ooo.Y);
		//DialogWindowWidget.Get()->DialogCreatorWindow222 = DialogCreatorWindow111.Get()->GetContent();
		//return DialogCreatorWindow111.ToSharedRef();
}
