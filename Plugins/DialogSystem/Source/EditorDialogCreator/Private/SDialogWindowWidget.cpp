...
#include "../../DialogSystem/Include/pugiconfig.hpp"
#include "../../DialogSystem/Include/pugixml.cpp"
...
void SDialogWindowWidget::Construct(const FArguments& InArgs)
{
	...
	ChildSlot		
		[
			//SAssignNew(DialogCreatorWindow, SBorder)
			SNew(SBorder)
			[			
				SNew(SOverlay)
				...
				[
					...
					[
						...
						[
							...
							SNew(SBox)
							.Visibility(this, &SDialogWindowWidget::GetButtonAddNewElement_Visibility)
							//.Visibility(EVisibility::Collapsed) //EVisibility::Visible
							.WidthOverride(32.f)
							.HeightOverride(32.f)
							[
								SNew(SButton)
						...
						[
							...
							[
								...
								.Text(this, &SDialogWindowWidget::GetCurrentDialogName)
						...
						[
							...
							[
								...
								.Font(FSlateFontInfo(FPaths::EngineContentDir() / TEXT("Slate/Fonts/Roboto-Regular.ttf"), 14 * SizeKoef))
								.Text(FText::FromString(FString("Finding Dialog")))
						...
						[
							...
							[
								...
								.Font(FSlateFontInfo(FPaths::EngineContentDir() / TEXT("Slate/Fonts/Roboto-Regular.ttf"), 14 * SizeKoef))
								.Text(FText::FromString(FString("Finding speech")))
					...
					[
						...
						.Visibility_Lambda([this]() -> EVisibility { return DialogContentVisibility; })  // Collapsed  Visible // EVisibility::Collapsed) //EVisibility::Visible
				...
				//  ================================================================
				//  ================================================================
				//  ==================  Helper Commentary Text  ====================
				//  ================================================================
				//  ================================================================
				+SOverlay::Slot()
				[
					SNew(SBox)
					.Visibility(this, &SDialogWindowWidget::GetHelperCommentaryVisibility)
					.Padding(TAttribute<FMargin>(this, &SDialogWindowWidget::GetHelperPanelPosition))
					//.Visibility(EVisibility::Visible)
					//.Padding(FMargin(20.f, 100))
					//.WidthOverride(100.f)
					//.HeightOverride(700.f)													
					//.HeightOverride(this, &SDialogWindowWidget::GetHelperCommentText_Height)
					//.HAlign(HAlign_Fill)
					//.VAlign(VAlign_Fill)
//					.HeightOverride(202.f)
//					.HeightOverride(this, &SDialogWindowWidget::GetHelperCommentText_Width)
					...
					[
						...
						.Padding(4.f)
						//.BorderBackgroundColor(FLinearColor(1.f, 0.25f, 0.f, 1))
						//.ColorAndOpacity(FLinearColor(0.f, 0.25f, 0.f, 0.7f))
						[
							//SNew(SScrollBox)

							//+ SScrollBox::Slot()
							//[
								SAssignNew(CommentaryTextBlock, STextBlock)
								.Margin(4.f)
								.Font(FSlateFontInfo(FPaths::EngineContentDir() / TEXT("Slate/Fonts/Roboto-Regular.ttf"), 14 * SizeKoef))
								.AutoWrapText(true)
								.WrappingPolicy(ETextWrappingPolicy::AllowPerCharacterWrapping)
								.ColorAndOpacity(FSlateColor(ColorUsualText))
								//.Text_Lambda([this]() -> FText { return FText::FromString(sCommentaryText); })
								//.Text(FText::FromString("AAAAAAAAAAAAAAAAAA000000"))
							//]
						]
					]
				]
	...
	//FVector2D FFFFFFF = DialogCreatorWindow.ToSharedRef()->ComputeDesiredSize(1);
...
void SDialogWindowWidget::Tick(const FGeometry& AllottedGeometry, const double InCurrentTime, const float InDeltaTime)
{
	...
	{ 
		...
		if (SApp.GetActiveTopLevelWindow())
		{
			isHelperMenuOpen = true;
			FVector2D _ClickPos = SApp.GetCursorPos();
			FVector2D _ViewportSize = SApp.GetActiveTopLevelWindow()->GetViewportSize();  //AllottedGeometry.GetAbsoluteSize();  
	...
	//   ========================   Draw   HelperCommentText at Cursor Position  ========================
	/*if (isHelperCommentaryMenuOpen)
	{
		...
		if (SlateAplication.GetActiveTopLevelWindow())
		{
			...
			//HelperCommentText_Height = 300.f;
			//HelperCommentText_Width = 400.f;
			// FVector2D 	ComputeDesiredSize(	float );
			//HelperPanelPosition = FMargin(_ClickPos.X, _ClickPos.Y, 0, 0);
			HelperPanelPosition = FMargin00.f, 0.f, 100.f, 100.f);
		}
	}
	*/
}
...
FText SDialogWindowWidget::GetCurrentDialogName() const
{
	int32 iStart = (*PathToCurrentXML).Find(FString("/"), ESearchCase::Type::IgnoreCase, ESearchDir::Type::FromEnd) + 1;    //  ESearchDir::Type::FromEnd, ESearchDir::Type::FromStart
	int32 iEnd = (*PathToCurrentXML).Find(FString("."), ESearchCase::Type::IgnoreCase, ESearchDir::Type::FromEnd);          // CaseSensitive
	int32 count = iEnd - iStart;

	return FText::FromString((*PathToCurrentXML).Mid(iStart, count));
}
...
EVisibility SDialogWindowWidget::GetButtonAddNewElement_Visibility() const
{
	return DialogSection.Num() > 0 ? EVisibility::Collapsed : EVisibility::Visible;
}
						...
						[ 
							...
							[
								...
								[
									...
									.OnMouseMove(this, &SDialogWindowWidget::OnMouseConcreteBorderHover, EProppertyToChange::DialogName)
									[
										SNew(SHorizontalBox)
							...
							[
								...
								[
									...
									[
										...
										.VAlign(VAlign_Fill)
										.BorderImage(PropertyDialog)
										.OnMouseMove(this, &SDialogWindowWidget::OnMouseConcreteBorderHover, EProppertyToChange::SpeechName)
										[
								...
								[
									...
									[
										...
										[
											...
											.OnMouseMove(this, &SDialogWindowWidget::OnMouseConcreteBorderHover, EProppertyToChange::ReplicNameCommentary)
											[
													SNew(STextBlock)
													.Font(FSlateFontInfo(FPaths::EngineContentDir() / TEXT("Slate/Fonts/Roboto-Regular.ttf"), 14 * SizeKoef))
										...
										[
											...
											.VAlign(VAlign_Fill)
											.BorderImage(PropertyReplic)
											.OnMouseMove(this, &SDialogWindowWidget::OnMouseConcreteBorderHover, EProppertyToChange::repeatReplic)
											[
									...
									[
										...
										[
											...
											[
												...
												.OnMouseMove(this, &SDialogWindowWidget::OnMouseConcreteBorderHover, EProppertyToChange::speker)
												[
									...
									[
										...
										[
											...
											[
												...
												.VAlign(VAlign_Fill)
												.BorderImage(PropertyResponse)
												.OnMouseMove(this, &SDialogWindowWidget::OnMouseConcreteBorderHover, EProppertyToChange::sound)
												[
									...
									[
										...
										[
											...
											[
												...
												.VAlign(VAlign_Fill)
												.BorderImage(PropertyResponse)
												.OnMouseMove(this, &SDialogWindowWidget::OnMouseConcreteBorderHover, EProppertyToChange::RepEvent)
												[
									...
									[
										...
										[
											...
											[
												...
												.VAlign(VAlign_Fill)
												.BorderImage(PropertyResponse)
												.OnMouseMove(this, &SDialogWindowWidget::OnMouseConcreteBorderHover, EProppertyToChange::time)
												[
									...
									[
										...
										[
											...
											[
												...
												.VAlign(VAlign_Fill)
												.BorderImage(PropertyResponse)
												.OnMouseMove(this, &SDialogWindowWidget::OnMouseConcreteBorderHover, EProppertyToChange::delay)
												[
								...
								[
									...
									[
										...
										[
											...
											.VAlign(VAlign_Center)
											.BorderImage(PropertyReplic)
											.OnMouseMove(this, &SDialogWindowWidget::OnMouseConcreteBorderHover, EProppertyToChange::ResponseNameCommentary)
											[
													SNew(STextBlock)
										...
										[
											...
											.OnMouseMove(this, &SDialogWindowWidget::OnMouseConcreteBorderHover, EProppertyToChange::repeatResponse)
											[
									...
									[
										...
										[
											...
											[
												...
												.VAlign(VAlign_Fill)
												.BorderImage(PropertyResponse)
												.OnMouseMove(this, &SDialogWindowWidget::OnMouseConcreteBorderHover, EProppertyToChange::DialogLink)
												[
									...
									[
										...
										[
											...
											[
												...
												.VAlign(VAlign_Fill)
												.BorderImage(PropertyResponse)
												.OnMouseMove(this, &SDialogWindowWidget::OnMouseConcreteBorderHover, EProppertyToChange::SpeechLink)
												[
									...
									[
										...
										[
											...
											[
												...
												.VAlign(VAlign_Fill)
												.BorderImage(PropertyResponse)
												.OnMouseMove(this, &SDialogWindowWidget::OnMouseConcreteBorderHover, EProppertyToChange::ResponseReturnType)
												[
							...
							[
								...
								[
									...
									[
										...
										SNew(SHorizontalBox)
										//.Visibility(this, &SDialogWindowWidget::GetVisibilityOfSectionPropperty_Static, EProppertyToChange::Response_variable_varName, CurrentPropertyIndex)  
										.Visibility_Lambda([=]() -> EVisibility
											{
											return *DialogSection[i_Dlg].SpeechSection[i_Speech]
												.ResponseSection[i_Response].VariableSection[i_ResponseVariable]
												.Response_VarSet_IncrDecr == FString("set")
												? EVisibility::Visible
												: EVisibility::Collapsed;
											})
										//DialogSection[i_Dlg].SpeechSection[i_Speech].ResponseSection[i_Response]
										//		.VariableSection[i_ResponseVariable].ChangeVariable_Value_Editable
										...
										[
											...
											.OnMouseMove(this, &SDialogWindowWidget::OnMouseConcreteBorderHover,
																	EProppertyToChange::Response_variable_varValue)
											[
									...
									//  ==============================================================================
										...
										//[
											SNew(SBorder)
											.Padding(GetPadding(7, 6, 7, 1))
											.HAlign(HAlign_Fill)
											.VAlign(VAlign_Fill)
											.BorderImage(ImageValueZone)      //  image  Condition-Property   "Variable-Value"  
											.OnMouseMove(this, &SDialogWindowWidget::OnMouseConcreteBorderHover, 
																		EProppertyToChange::Response_variable_IncrDecr)
											[
												SAssignNew(DialogSection[i_Dlg].SpeechSection[i_Speech].ResponseSection[i_Response]
															.VariableSection[i_ResponseVariable].CB_IncrDecr, SComboBox<TSharedPtr<FString>>)
												.ContentPadding(GetPadding(4.0f, 0, 4.0f, 5.0f))
												.OptionsSource(&ResponseVariableSetting_InctDecr_ComboList)
												//.OnComboBoxOpening( this, &SDialogWindowWidget::OnSpekerComboBoxOpening, CurrentPropertyIndex)
												.OnGenerateWidget(this, &SDialogWindowWidget::HandleComboBoxGenerateWidget)
												.OnSelectionChanged(this, &SDialogWindowWidget::GetResponseVarSet_InctDecr_ComboList, CurrentPropertyIndex)
												//.InitiallySelectedItem(DialogSection[i_Dlg].SpeechSection[i_Speech].ReplicSection[i_Replic].RepSection[i_Rep].CurrentSpiker)
												[

													SNew(STextBlock)
													.Margin(0)
													.Text(this, &SDialogWindowWidget::GetCurrentResponseVarSet_InctDecr, CurrentPropertyIndex)
													.Font(FSlateFontInfo(FPaths::EngineContentDir() / TEXT("Slate/Fonts/Roboto-Regular.ttf"), 12 * SizeKoef))
													.ColorAndOpacity(FLinearColor(0.0f, 0.0f, 0.08f, 1))
												]//  ComboBox
											]
...
void SDialogWindowWidget::GetResponseVarSet_InctDecr_ComboList(TSharedPtr<FString> NewSelection, ESelectInfo::Type SelectInfo, FPropertyIndex PropertyIndex)
{
	DialogSection[PropertyIndex.iDialog].SpeechSection[PropertyIndex.iSpeech].ResponseSection[PropertyIndex.iResponse]
											.VariableSection[PropertyIndex.iResponseVariable].Response_VarSet_IncrDecr = NewSelection;
}
//FText SDialogWindowWidget::GetCurrentResponseRepeatLabel(FPropertyIndex PropertyIndex) const
FText SDialogWindowWidget::GetCurrentResponseVarSet_InctDecr(FPropertyIndex PropertyIndex) const
{
	return DialogSection[PropertyIndex.iDialog].SpeechSection[PropertyIndex.iSpeech].ResponseSection[PropertyIndex.iResponse].ResponseRepeatPropperty.IsValid() ?
		FText::FromString(*DialogSection[PropertyIndex.iDialog].SpeechSection[PropertyIndex.iSpeech]
			.ResponseSection[PropertyIndex.iResponse].VariableSection[PropertyIndex.iResponseVariable].Response_VarSet_IncrDecr) :
		FText::GetEmpty();
}
...
void SDialogWindowWidget::InitComboBox_StaticList()
{
	...
	ResponseVariableSetting_InctDecr_ComboList.Add(MakeShareable(new FString(TEXT("set"))));
	ResponseVariableSetting_InctDecr_ComboList.Add(MakeShareable(new FString(TEXT("Increment"))));
	ResponseVariableSetting_InctDecr_ComboList.Add(MakeShareable(new FString(TEXT("Decrement"))));
						...
						ContentToSave += FString("\t\t\t<response repeat=\"") +
							...
							*DialogSection[i_Dlg].SpeechSection[i_Speech].ResponseSection[i_response].ResponseReturnTypePropperty +
																...
																.ResponseSection[i_response].VariableSection.Num(); i_response_variables++)
							{
								...
								ContentToSave += FString("\" IncrDecr=\"") +
									*DialogSection[i_Dlg].SpeechSection[i_Speech].ResponseSection[i_response]
									.VariableSection[i_response_variables].Response_VarSet_IncrDecr +
								FString("\">") +
								DialogSection[i_Dlg].SpeechSection[i_Speech].ResponseSection[i_response]
...
void SDialogWindowWidget::ChoiseLoadingXmlFile()
{
	...
	if (GEngine)  // && GEngine->GameViewport -- In Runtime
	{
		...
		if (DesktopPlatform)
		{
			...
			// open DialogWindow to Save
			uint32 SelectionFlag = 0; // if one or few files has chosen
			DesktopPlatform->OpenFileDialog(ParentWindowHandle, FString("Load file (.xml)"), Path, FileName, FString(".xml"), SelectionFlag, OutFileNames);
			///GEngine->AddOnScreenDebugMessage(-1, 3.f, FColor::Orange, OutFileNames[0]);
			...
			for (int32 i_Response = 0; i_Response < DLG_ResponseNode.size(); i_Response++)
			{
				...
				*ResponseTmp.ResponseReturnTypePropperty = ResponseReturnTypeStr;
				...
				for (int32 i_ResponseVariable = 0; i_ResponseVariable < DLG_ResponseVariableNode.size(); i_ResponseVariable++)
				{
					...
					std::string STD_ResponseVar_IncrDecr(DLG_ResponseVariableNode[i_ResponseVariable].node().attribute("IncrDecr").as_string());  //   get attribute("IncrDecr")
					FString ResponseVar_IncrDecr(STD_ResponseVar_IncrDecr.c_str());

					*ResponseVariableTmp.Response_VarSet_IncrDecr = ResponseVar_IncrDecr;
...
/*FVector2D SDialogWindowWidget::GetHelperPanelSize() const
{
	return HelperPanelSize;
}*/
...
FReply SDialogWindowWidget::OnMouseConcreteBorderHover(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent, EProppertyToChange _ProppertyToChange)
{
	//UE_LOG(LogTemp, Warning, TEXT("  >>>>>>>>>>> HOVER  %f"), gCurrentTimeToHelperTextDraven);

	if (gCurrentTimeToHelperTextDraven <= 0)
	{
		//UE_LOG(LogTemp, Warning, TEXT("  >>>>>>>>>>> HOVER  OkOkOkOk"));
		isHelperCommentaryMenuOpen = true;
		CurrentGeometry = const_cast<FGeometry*>(&MyGeometry);



		FString sCommentaryText = GetCommentaryText(_ProppertyToChange);
		CommentaryTextBlock.Get()->SetText(sCommentaryText);
		...
		if (SlateAplication.GetActiveTopLevelWindow())
		{
			...
			if (SApp.GetActiveTopLevelWindow())
			{
				//FVector2D _ViewportSize = SlateAplication.GetActiveTopLevelWindow()->GetViewportSize();  // FullScreen Position      //AllottedGeometry.GetAbsoluteSize();
				FVector2D DlgPanelPos = this->GetTickSpaceGeometry().GetAbsolutePosition();             // ourWindowPanel position
				FVector2D MousePos = SApp.GetCursorPos();									// cursor Pos in fullScreen
				FVector2D LocalMousePos = MousePos - DlgPanelPos;
				FVector2D DlgPanelSize = this->GetTickSpaceGeometry().GetLocalSize();// GetAbsoluteSize();
				float BottomPadding = 0;

				FVector2D TextSizeTmp = CommentaryTextBlock.Get()->ComputeDesiredSize(1);
				BottomPadding = DlgPanelSize.Y - TextSizeTmp.Y;  
				 
				//UE_LOG(LogTemp, Warning, TEXT("Mouse locale Pos,      x = %f   y = %f"), LocalMousePos.X, LocalMousePos.Y);
				HelperPanelPosition = FMargin(0.f, 0.f, 0.f, BottomPadding - 5.f); 
			}
	...
	gCurrentTimeToHelperTextDraven -= 0.1f;
	return FReply::Handled();
}


FReply SDialogWindowWidget::OnMouseMove(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
	if (gCurrentTimeToHelperTextDraven != gTimeToHelperTextDraven)	gCurrentTimeToHelperTextDraven = gTimeToHelperTextDraven;
	else if(CurrentGeometry			   != &MyGeometry)				isHelperCommentaryMenuOpen = false;

	return FReply::Handled();
}


EVisibility SDialogWindowWidget::GetHelperCommentaryVisibility() const
{
	if (isHelperCommentaryMenuOpen) return EVisibility::Visible;
	else							return EVisibility::Collapsed;

}

FString SDialogWindowWidget::GetCommentaryText(EProppertyToChange _ProppertyToChange)
{

	// ============= (Comentary)  DIALOG-Name
	if (_ProppertyToChange == EProppertyToChange::DialogName)
	{
		return FString("\
Name of Dialog-section.\n\
It can be used for linking betwean dialog branch\n\
\n\
First dialog should named as \"DEFAULT\".\n\
");
	}
	// ============== (Comentary)  SPEECH
	else if (_ProppertyToChange == EProppertyToChange::SpeechName)
	{
		return FString("\
Name of Speech-section.\n\
\n\
It can be used for linking betwean dialog branch\n\
if link's name was \"DEFAULT\", you will go through all Speeches with name = \"DEFAULT\" and choose first of them suitable by condition\n\
\n\
Dialog-section can content a lot of Speech-section. Speech-section content Replic and Response.\n\
In game will play all allowed Replic-section(one by one) and then show all allowed Response-Section\n\
");
	}	
	else if (_ProppertyToChange == EProppertyToChange::ReplicNameCommentary)
	{
		return FString("\
One replic express only one dialog-phrase.\n\
Replic-section may content a lot of \"phrases variant\", but only one from them will be choosen (which suitable by conditions).\
If more then one \"phrases\" are true, it will be choosen randomly.\n\
");		
	}
	else if (_ProppertyToChange == EProppertyToChange::repeatReplic)
	{
		return FString("\
* \"once\" meant this Replic-block play only at first time on this Speech-block.\n\
		   Even if block didn't play, due to condition, next time, when we come here it will missed.\n\
\n\
* \"after once\" will play when we come to this Speech-block second or more time. At first time it will missed\n\
\n\
**Also \"once\" or \"after once\" will play when we come here from other Dialog-branch with return-type \"click\" or \"pass\".\n\
\n\
* \"back\" it will play when we come here from other Dialog-branch with return-type \"back\" \n\
\n\
* \"always\" it will always playing \
.\n\
");
	}
	//  =============== (Comentary) Rep  ================
	...
	{
		return FString("\
This section set real resource (Speker name, image and sound) showing in game \n\
\n\
     Speker name \n\
\n\
You should create speker in \"Speker-panel\" and choose his here\n\
");
	}
	else if (_ProppertyToChange == EProppertyToChange::sound)
	{
...
This section set real resource (Speker name, image and sound) showing in game \n\
\n\
     Speker's sound for this replic\n\
\n\
You should create a sounds for current speker in \"Speker-panel\" and choose it here\n\
");
	}
	else if (_ProppertyToChange == EProppertyToChange::RepEvent)
	{
...
\n\
     return EVENT - string\n\
\n\
This \"String\" will return in BP when this \"Phrase-variant\" calling. \
You can catch the EVENT, recognize by \"string-value\" and do anything if need. \
");
	}
...
\n\
     Delay time\n\
\n\
waiting oprion to next phrase n\
");
	}
	else if (_ProppertyToChange == EProppertyToChange::delay)
	{
		return FString("\
This section set real resource (Speker name, image and sound) showing in game \n\
\n\
Delay time (after phrase) if need\n\
");
	}
	//+++++++++++++++++++++++++++++++
//  =============== (Comentary) Response  ================
	else if (_ProppertyToChange == EProppertyToChange::ResponseNameCommentary)
	{
		return FString("\
\"Response\" - it's what player can replay to NPC-speker\n\
\n\
Response can change variables whe will selectes\
");
	}
	else if (_ProppertyToChange == EProppertyToChange::repeatResponse)
	{
	return FString("\
Response can be shown always or hidden after first activated\n\
");
	}
	else if (_ProppertyToChange == EProppertyToChange::DialogLink)
	{
	return FString("\
Next dialog link's name (go there after player select this response)\n\
");
	}
	else if (_ProppertyToChange == EProppertyToChange::SpeechLink)
	{
	return FString("\
Next speech link's name (go there after player select this response)\n\
\n\
If \"DEFAULT\", go through all Default-speech and stop on first allowed\
");
	}
	else if (_ProppertyToChange == EProppertyToChange::ResponseReturnType)
	{
	return FString("\
* click_... - meant this Response-section will showen and will allow for click\n\
\n\
* pass_... - Response wou't showen (but will applay automaticly) \n\
                  if there are variables to change in the response, they will change automatically \n\
                  if there is only one Response is Speech-section, there will Link automatically\n\
\n\
* ..._[empty] - Link to (Dialog->Speech->...) and consistently play all Replic-sections with repeat-parameter: \"once\" or \"after once\" (\"always\" will playing too)\n\
\n\
* ..._back - Link to (Dialog->Speech->...) and consistently play all Replic-sections with repeat-parameter: \"back\" (\"always\" will playing too)\n\
\n\
* ..._noreplic - Link to (Dialog->Speech->...) but passing all Replics. Show only Responses\
				(so player will see last replics from last dlg-branch but new Responses from new branch)\n\
");
	}
	   
	//  =============== (Comentary) Response Set Variable  ================
	else if (_ProppertyToChange == EProppertyToChange::Response_variable_varValue)
	{
	return FString("\
...
");
	...
	else if (_ProppertyToChange == EProppertyToChange::Response_variable_IncrDecr)
	{
	return FString("\
* \"Set\" - you should write value as string\n\
\n\
* \"Increment\" or \"decrement\" - if value has number at the end \"xx\" or \"-xx\", it will Increment or decrement \n\
	if hasn't one it will createt the number \"0\" at the end, and increase or decrease it\n\
");
	}
...
//FOptionalSize SDialogWindowWidget::GetHelperCommentText_Height() const
//{
//	return FOptionalSize(HelperCommentText_Height);
//}
//FOptionalSize SDialogWindowWidget::GetHelperCommentText_Width() const
//{
//	return FOptionalSize(HelperCommentText_Width);
//}

//  =========================================================================================================
//  =========================================================================================================
//  ===============================    Helper Commentary Text     (Finish)    ===============================
//  =========================================================================================================
//  =========================================================================================================
